...
import { classMap, consume, css, customElement, html, LitElement, map, state } from "../deps/lit.ts";
...
export class WakutetStageControllerElement extends LitElement {
  ...
  @state()
  private _playerHearts: HeartsData[] = [null, null].map(_ => ({ lit: 0, max: 0 }));
  @state()
  ...
  async firstUpdated() {
    ...
    this._playerProfilesVisibleReplicant = await client.getReplicant("playerProfilesVisible");
    ...
    this._playerHeartsReplicant.subscribe(value => {
      this._playerHearts = value ?? [null, null].map(_ => ({ lit: 0, max: 0 }));
    });
  ...
  private _changeNumLitHearts(playerIndex: number, delta: number) {
    ...
    playerHearts[playerIndex] = { ...playerHearts[playerIndex], lit: Math.min(Math.max(playerHearts[playerIndex].lit + delta, 0), playerHearts[playerIndex].max) };
  ...
  render() {
    const numMaxHearts = Math.min(Math.max(this._playerHearts[0]?.max ?? 0, 0), 5);
    return html`
    <div class="container">
      <fluent-select value=${numMaxHearts} @change=${(ev: Event) => this._setNumMaxHearts(Number((ev.target as FluentSelect).value))}>
      ...
      <div>
        ${map(this._playerHearts, (e, i) => html`
        <div>
          ${map([...new Array(this._playerHearts[i].max)], (_, j) => html`<span class=${classMap({ "heart": true, "heart-lit": j < this._playerHearts[i].lit })}>♥</span>`)}
          <fluent-button @click=${() => this._changeNumLitHearts(i, +1)}>+</fluent-button>
          <fluent-button @click=${() => this._changeNumLitHearts(i, -1)}>-</fluent-button>
          <fluent-button @click=${() => this._changeNumLitHearts(i, -Infinity)}>0</fluent-button>
        </div>
        `)}
      </div>
      ${map(this._playerNames, (e) => html`${e}`)}
      <fluent-checkbox ?checked=${this._playerProfilesVisible} @change=${(ev: Event) => this._setPlayerProfilesVisible((ev.target as HTMLInputElement).checked)}>プロフィールを表示</fluent-checkbox>
    </div>
    `;
  }
}
